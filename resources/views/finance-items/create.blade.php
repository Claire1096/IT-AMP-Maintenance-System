...
@section('title', 'Add Finance Item')

@section('content')
    <div class="flex justify-between items-start mb-6">
        <div>
            <h1 class="text-lg font-bold">ADD ITEM</h1>
            <p class="text-xs text-gray-400">Audit / Add new item</p>
        </div>
    ...
    <form method="POST" action="{{ route('finance-items.store') }}" class="max-w-2xl">
        @csrf
        <div class="bg-white border border-rose-100 rounded-xl p-5 shadow-sm">
            ...
            <div class="grid grid-cols-2 gap-3 mb-3">
                <div>
                    ...
                    <input type="number" name="quantity" value="{{ old('quantity', 0) }}" min="0" required class="w-full text-xs border-gray-300 rounded-md">
                </div>
                <div>
                    <label class="block text-[10px] font-semibold text-gray-500 mb-1">CURRENT QUANTITY *</label>
                    <input type="number" name="current_quantity" value="{{ old('current_quantity', 0) }}" min="0" required class="w-full text-xs border-gray-300 rounded-md">
                </div>
            </div>
