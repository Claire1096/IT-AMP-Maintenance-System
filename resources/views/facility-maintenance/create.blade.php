...
@section('content')
    <div class="flex justify-between items-start mb-6">
        <div>
            <h1 class="text-lg font-bold">MAINTENANCE</h1>
            <p class="text-xs text-gray-400">Maintenance / Schedule Maintenance</p>
        </div>
    ...
    <form method="POST" action="{{ route('facility-maintenance.store') }}">
        @csrf
            ...
            <div class="bg-white border border-rose-100 rounded-xl p-5 shadow-sm">
                ...
                <div class="mb-3">
                    ...
                    <select name="facility_item_id" required class="w-full text-xs border-gray-300 rounded-md">
                        ...
                        @foreach ($items as $facilityItem)
                            <option value="{{ $facilityItem->id }}" @selected(old('facility_item_id', $selectedItemId) == $facilityItem->id)>
                ...
                <div class="mb-3">
                    ...
                    <select name="maintenance_type" required class="w-full text-xs border-gray-300 rounded-md">
                        ...
                        @foreach ($maintenanceTypes as $type)
                            <option value="{{ $type }}" @selected(old('maintenance_type') === $type)>{{ ucfirst($type) }} Maintenance</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3" placeholder="Add description / remarks / notes..."
                              class="w-full text-xs border-gray-300 rounded-md">{{ old('notes') }}</textarea>
                </div>
            </div>
            ...
            <div class="bg-white border border-rose-100 rounded-xl p-5 shadow-sm flex flex-col justify-between">
                <div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="technician" value="{{ old('technician') }}" placeholder="Enter technician name..."
                               class="w-full text-xs border-gray-300 rounded-md">
                    </div>
                        ...
                        @foreach ($checklistItems as $item)
                            <label class="flex items-center gap-2 text-xs text-gray-600">
                                <input type="checkbox" name="checklist[]" value="{{ $item }}"
                                       @checked(collect(old('checklist', []))->contains($item))>
                                {{ $item }}
                            </label>
                        @endforeach
                        <label class="flex items-center gap-2 text-xs text-gray-600">
