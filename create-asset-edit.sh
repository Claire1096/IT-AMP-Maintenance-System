...
cat > resources/views/assets/edit.blade.php << 'BLADE_EOF'
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
            ...
            <form method="POST" action="{{ route('assets.update', $asset) }}" class="max-w-3xl">
                ...
                <div class="bg-white border border-rose-100 rounded-xl p-5 shadow-sm mb-6">
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="2" class="w-full text-xs border-gray-300 rounded-md">{{ old('notes', $asset->notes) }}</textarea>
                    </div>
                </div>
...
BLADE_EOF
echo 'assets/edit.blade.php created!'
